    ...
    if scatter == True:
      ...
      if hist == True:
        if hist2D == True:
          ...
          if len(y) == 1:
            if x_range <> None:
              ax.hist(y[0],color='red', label=labels[0],bins=30, range=[x_range[0], x_range[1]],
            histtype='step', alpha=0.5)
            else:
              ax.hist(y[0],color='red', label=labels[0],bins=30,
            histtype='step', alpha=0.5)
          else:
            ...
            if x_range <> None:
              ...
              for i,ys in enumerate(y): 
                ax.hist(ys,color=colors_rgb[i],label=labels[i],bins=30,histtype='stepfilled',normed=1, alpha=0.5) 
